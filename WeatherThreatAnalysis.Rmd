...
R code visible. In normal circumstances I would suppress much of it
with `echo = FALSE`_
...
risks. For example, it would be surprising to see blizzards playing a
major role in Florida fatalities. The maps below higlight the top
threats in each of the four casuality categories in each of the
continental 48 states. Only the main national risks are specifically
...
* Scientific notation (eg `1e+6`) appears to always be considered
  numeric by R, even if there is no decimal (stops on read.table if it
